...
if st.button("✨ 영상 제작"):
    if files and logo:
        with st.spinner('블랙 화면 제거 및 음악 페이드 적용 중...'):
            try:
                processed_clips = []
                for i, f in enumerate(files):
                    clip = process_item(f)
                    if i == 0:
                        # 첫 장면: 블랙 방지를 위해 페이드 인 없이 시작, 끝만 페이드 아웃
                ...
                if bgm:
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as mt:
                        ...
                        audio = audio.fadein(1.0).fadeout(2.0)
                        final = final.with_audio(audio)
                
                # 최종 파일 저장
                output_name = "biyoil_final_perfect.mp4"
                final.write_videofile(output_name, fps=24, codec="libx264", audio_codec="aac")
                
                st.video(output_name)
                st.success("🎉 완벽한 영상이 완성되었습니다! 저장해서 사용하세요.")
            except Exception as e:
                st.error(f"제작 중 에러 발생: {str(e)}")
    else:
        st.error("사진과 로고를 모두 올려줘!")
